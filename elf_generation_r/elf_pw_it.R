...
elf_pw_it <- function(inputs, data, x_metric_code, y_metric_code, ws_ftype_code, Feature.Name_code, Hydroid_code, search_code, token, nhdplus_flowmetric_value, startdate, enddate){
  ...
  if(nrow(data) && length(duplicates) > 3) {   
    ...
    skipper <- paste("Error:\n\n (No breaks are found using this piece-wise method in \n", search_code,")", sep='')
    elf_skip (save_directory,skipper)
  }
