...
elf_retrieve_data <- function(inputs = list()){
  ...
  #RETRIEVE FLOWMETRIC VALUE
  if (x_metric == "nhdplus_point_sqkm") {
    uri <- paste(site,"nhdplus-point-export2","279034",sep="/")
    nhdplus_point_data <- read.csv(uri, header = TRUE, sep = ",")
    nhdplus_flowmetric_value <- nhdplus_point_data$area_sqkm
  } else {
    #-----------------------------------------------
    #RETRIEVE NHDPLUS HYDROID
    nhdplus_hydroid <- paste(site,"nhdplus_point_hydroid","279034",sep="/")
    nhdplus_hydroid <- read.csv(nhdplus_hydroid, header = TRUE, sep = ",")
    nhdplus_hydroid <- nhdplus_hydroid$nhdplus_hydroid 
    
    #RETRIEVE NHDPLUS FLOWMETRIC VALUE
    nhdplus_flowmetric_value <- paste(site,"point-analysis-flowmetric-value",nhdplus_hydroid,x_metric,sep="/")
    nhdplus_flowmetric_value <- read.csv(nhdplus_flowmetric_value, header = TRUE, sep = ",")
  ...
  if(ws_ftype == "hwi_region"){
    bundle <- "ecoregion"
  } else if(ws_ftype == "state") {
    bundle <- "landunit" 
  } else if(ws_ftype == "ecoregion_iii") {
    bundle <- "ecoregion" 
  } else if(ws_ftype == "ecoregion_iv") {
    bundle <- "ecoregion" 
  } else if(ws_ftype == "ecoiii_huc6") {
    bundle <- "ecoregion" 
  } else {
    bundle <- "watershed" 
  }
  ...
  if (!(target_hydrocode == '')) {
    HUClist_url_full <- paste(HUClist_url_base, ws_ftype, target_hydrocode, sep = "/");
  } else {
    HUClist_url_full <- paste(HUClist_url_base, ws_ftype, sep = "/");
  }
  ...
  for (j in offset_x_metric:length(x_metric)) {
    ...
    for (i in offset_hydrocode:length(Watershed_Hydrocode)) {
      ...
      if(nrow(data) <= 3) {
        ...
        skipper <- paste("Error:\n\n (fewer than 3 datapoints in ", Watershed_Hydrocode[i],")",sep='')
        #skipper <- "NOT ENOUGH DATA"
      ...
      if(length(remove_da_duplicates) == 1 | length(remove_da_duplicates) == 2) {
        ...
        skipper <- paste("Error:\n\n (the points are all organized in 1 or 2 vertical lines in ", Watershed_Hydrocode[i],")", sep='')
        elf_skip (save_directory,skipper)
        next 
      ...
      if(length(remove_metric_duplicates) == 1 | length(remove_metric_duplicates) == 2) {
        ...
        skipper <- paste("Error:\n\n (the points are all organized in 1 or 2 horizontal lines in ", Watershed_Hydrocode[i],")", sep='')
        elf_skip (save_directory,skipper)
        next 
      } #closes bar of points skip if-statement (rare)
